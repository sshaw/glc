...
var (
	eventTypes = []string{ Issue, IssueComment, PullRequest /*, PullRequestComment */ }
	ignoreFiles = []string{
...
func (glc *GLC) initOptions(options *EventOptions) {
	...
	glc.ignoreFiles = arrayToMap(ignoreFiles)
...
func (glc *GLC) ignoreFile(url *GitHubURL) bool {
...
func (glc *GLC) ignoreEvent(event *Event) bool {
	if event == nil ||
		glc.eventHasBeenProcessed(event.eventID) ||
		glc.ignoreType(event.Type) ||
		glc.ignoreUser(event.Actor) ||
		glc.ignoreRepo(event) {
